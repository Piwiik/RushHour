...
class Board():
    ...
    def copy(self):
        ...
        for cell in cells :
            if cells[cell]!=None :
                if not self.get_starting_cell(cell) in done :
                    cell = self.get_starting_cell(cell)
                    done.append(self.get_starting_cell(cell))
                    vehicles.append((cells[cell].get_orientation(), cells[cell].get_name(), cell))
